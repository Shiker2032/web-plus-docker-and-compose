...
export const UserPage = ({ extraClass = "" }) => {
  ...
  return (
    <section className={`${styles.content} ${extraClass}`}>
      <ButtonReturn />
      <h1 className={`text text_type_h1 text_color_primary mb-16 ${styles.title}`}>
        {`Пользователь: ${anotherUser?.username} `}
      </h1>

      <p className={`text text_type_main text_color_primary mb-16 ${styles.description}`}>
        {`Обо мне: ${anotherUser?.about || "ничего неизвестно"}`}
      </p>
      <div className={styles.cards}>
        <h2 className={`text text_type_h2 text_color_primary ${styles.title}`}>Вишлист пользователя:</h2>
        <div className={styles.cards_box}>
          {anotherUserWishes?.map(({ price, name, raised, image, link, id }) => {
            return (
              <GoodCard
                key={id}
                id={id}
